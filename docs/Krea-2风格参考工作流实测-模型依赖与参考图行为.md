...
**想同时给"人物参考图"和"画风参考图"怎么办？** 工作流只接了 `image1`，但
`TextEncodeQwenImageEditPlus` 本身有 `image1/image2/image3` 三个图像输入，
在 API prompt 里补一个 `LoadImage` 节点接到 `image2` 上即可。注意 `image2` 是 IMAGE 型输入，
**必须给节点连线，不能塞文件名字符串**，否则报 `'str' object has no attribute 'movedim'`。
接法、效果对比和多视角设定图的裁剪技巧见
[ComfyUI多参考图接入方式](ComfyUI多参考图接入方式.md)。
...
**风格参考这条路建议 20 步起步**——实测 8 步会出现线条发糊和块状伪影，
细节（流苏、纹样、配饰）基本丢失，20 步才到可交付水平。
